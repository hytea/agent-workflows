...
2. **setup** (haiku). Creates a **git worktree** off the **local** `development` branch for this bead (`isolation: 'worktree'`). Forking off local `development` (not `origin/development`) lets each ticket compound on prior tickets already merged into local `development` during the run, minimizing same-file merge conflicts; the supervisor brings local `development` current with origin once at run start (fast-forward only). Commits the spec. Transitions the bead to `in_progress` (idempotent — supervisor already claimed it). Reports branch + HEAD sha. Does not implement.
