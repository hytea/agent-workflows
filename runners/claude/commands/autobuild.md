...
6. **Build the wave.** Invoke `Workflow({ name: 'autobuild:autobuild-supervisor', args: { config, profile, beads: <wave>, nowMs: Date.now() } })` (`nowMs` is a reserved seam — the supervisor does no time math today; all wakeup arithmetic lives in this command turn). **Immediately persist the `runId` returned by the `Workflow` tool result to the state file as `lastRunId`** so a wakeup can resume it. If the tool result does not surface a `runId`, treat overnight resume as unavailable for this run and log that — do not silently proceed assuming resume will work.
